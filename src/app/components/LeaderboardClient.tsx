...
export default function LeaderboardClient({ initialWeek, initialRows }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (av === bv) {
        // tie-breaker: goals desc, then player_points desc, then user_id asc
          ...
          <thead className="sticky top-0 bg-white/90 backdrop-blur">
            <tr className="[&>th]:py-3 [&>th]:px-3 [&>th]:text-left [&>th]:font-semibold">
              ...
              <SortableTH
                ...
                dir={sortDir}
                onClick={() => handleHeaderClick("total_points")}
              >
                Total Pts
              </SortableTH>
              <SortableTH
              ...
              <SortableTH
                active={sortKey === "player_points"}
                dir={sortDir}
                onClick={() => handleHeaderClick("player_points")}
              >
                Skater Pts
              </SortableTH>
              <SortableTH
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r, idx) => (
                <tr
                  key={`${r.user_id}-${idx}`}
                  className="border-t hover:bg-gray-50/60 [&>td]:py-2.5 [&>td]:px-3"
                >
                  <td className="tabular-nums">{idx + 1}</td>
                  <td className="font-semibold tabular-nums">
                    {r.total_points ?? 0}
                  </td>
                  <td className="tabular-nums">{r.goals ?? 0}</td>
                  <td className="tabular-nums">{r.player_points ?? 0}</td>
                  <td className="tabular-nums">{r.assists ?? 0}</td>
...
function SortableTH({
  ...
  return (
    <th>
      <button
        onClick={onClick}
        className={`inline-flex items-center gap-1 rounded-md px-1 py-0.5 transition hover:bg-gray-100 ${
          active ? "font-bold" : ""
        }`}
        aria-pressed={active}
      >
